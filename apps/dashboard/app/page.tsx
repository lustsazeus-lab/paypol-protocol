...
export default function Dashboard() {
    ...
    const connectWallet = useCallback(async () => { try { const accounts = await (window as any).ethereum.request({ method: 'eth_requestAccounts' }); await ensureTempoNetwork(); await initializeSession(accounts[0]); } catch (error) { showToast('error', 'Connection request declined.'); } }, [showToast, ensureTempoNetwork]);
